...
namespace Wulkanizacja.Service.Core.Aggregates
{
    public class TireAggregate : AggregateRoot
    {
        ...
        public TireAggregate(TireModel tireModel, AggregateId id, DateTimeOffset? createDate, DateTimeOffset? editDate)
        {
            ...
            Type = tireModel.TireType;
            ManufactureDate = tireModel.ManufactureDate;
            CreateDate = (createDate ?? DateTimeOffset.UtcNow).ToLocalTime();
            EditDate = (editDate ?? CreateDate).Value.ToLocalTime();
            Comments = tireModel.Comments;
            QuantityInStock = tireModel.QuantityInStock;
        }
                     ...
                     string? manufactureDate, string? comments, int quantityInStock)
        {
            ...
            SpeedIndex = speedIndex ?? string.Empty;
            LoadIndex = loadIndex ?? string.Empty;
            Type = tireType.HasValue ? (TireType)tireType.Value : TireType.Summer;
            ManufactureDate = manufactureDate ?? string.Empty;
            EditDate = DateTimeOffset.UtcNow;
            Comments = comments;
        ...
        public TireAggregate UpdateTire(TireAggregate originalTireAggregate)
        {
            AddDomainEvent(new UpdateTireEvent(this, originalTireAggregate));
            return this;
        }
